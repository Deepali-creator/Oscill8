...
var Countries = require('app_oscill8/cartridge/scripts/util/Countries');
...
function initAddressForm(cart) {
    ...
    dw.system.Logger.warn('Here Intializing Billing Form...');
    ...
    if (app.getForm('singleshipping').object.shippingAddress.useAsBillingAddress.value === true) {
        dw.system.Logger.warn('IF Check......');
        dw.system.Logger.warn( app.getForm('singleshipping').object.shippingAddress.addressFields.shippingEmail.value);
        ...
        app.getForm('billing').object.billingAddress.email.emailAddress.value = app.getForm('singleshipping').object.shippingAddress.addressFields.shippingEmail.value;
        app.getForm('billing').object.billingAddress.addressFields.phone.value = app.getForm('singleshipping').object.shippingAddress.addressFields.phone.value;
    } else if (cart.getBillingAddress() !== null) {
        dw.system.Logger.warn('Here It is not consider');
        ...
        app.getForm('billing').object.billingAddress.email.emailAddress.value = billingAddress.custom.email;
    } else if (customer.authenticated && customer.profile.addressBook.preferredAddress !== null) {

        dw.system.Logger.warn('Customer True');
        dw.system.Logger.warn(customer.profile.addressBook.preferredAddress.custom.email);
        ...
        app.getForm('billing').object.billingAddress.email.emailAddress.value = customer.profile.addressBook.preferredAddress.custom.email;
    }
}
...
function startEdit(cart, params) {
...
function publicStart() {
   dw.system.Logger.warn("========in CoBilling publicStart=========");
    ...
    if (cart) {
        ...
        Logger.warn('applicablePaymentMethods '+applicablePaymentMethods);
        ...
        if (paymentMethods.valid) {
            ...
            Logger.warn('paymentMethods '+paymentMethods.selectedPaymentMethodID.value);
...
function publicStartEdit() {
    ...
    if (cart) {
     ...
     dw.system.Logger.warn('credit cart list '+creditCardList);
     dw.system.Logger.warn('applicablePaymentMethods '+applicablePaymentMethods);
        ...
        startEdit(cart, {ApplicableCreditCards: creditCardList.ApplicableCreditCards});
    } else {
        app.getController('Cart').Show();
    }
}
...
function handlePaymentSelection(cart) {
    var result;
    Logger.warn("=============In CoBilling handlePayments==============");
    ...
    if (empty(PaymentMgr.getPaymentMethod(app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value).paymentProcessor)) {
        ...
        Logger.warn('trueeeeeeee');
    }
    if (!result) {
        Logger.warn('In handle-------------');
        ...
        Logger.warn('---MethodName '+app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value);
    }
...
function handleBillingAddress(cart) {
    ...
    Transaction.wrap(function () {
        ...
        if (!billingAddress) {
            ...
            Transaction.wrap(function () {
               billingAddress.custom.email = app.getForm('billing').object.billingAddress.email.emailAddress.value;
            });
        }
 ...
 */
function billing() {
   dw.system.Logger.warn("========in CoBilling Billing=========");
    var billingForm = app.getForm('billing').object;
    var paymentMethods = billingForm.paymentMethods;
    Logger.warn('paymentMethods '+paymentMethods.selectedPaymentMethodID.value);
    var paymentMethodID = paymentMethods.selectedPaymentMethodID.value;
    Logger.warn('paymentMethodID ' +PaymentMgr.getPaymentMethod(paymentMethodID).getPaymentProcessor().getID());
        ...
        save: function () {
            Transaction.wrap(function () {
                ...
                handlePaymentSelection(cart).error) {
                    ...
                    var PaymentSelectionResult = handlePaymentSelection(cart);
                  
                   Logger.warn('-----------PaymentSelectionResult--------------'+PaymentSelectionResult);
...
function validatePayment(cart) {
   dw.system.Logger.warn("========in CoBilling validatePayment=========");
   dw.system.Logger.warn("CUSTOMER  *********"+customer);
    ...
    if (app.getForm('billing').object.fulfilled.value) {
        ...
        // Logger.warn('invalidPaymentInstruments----------------------'+PaymentMgr.getPaymentMethod(invalidPaymentInstruments[0].getPaymentMethod()));
        if (!invalidPaymentInstruments && cart.calculatePaymentTransactionTotal()) {
            dw.system.Logger.warn("---------result = true;");
            result = true;
        } else {

            app.getForm('billing').object.fulfilled.value = false;
            dw.system.Logger.warn("countryCode  *********"+countryCode);
            dw.system.Logger.warn("paymentAmount.value  *********"+ paymentAmount.value);
            result = false;
        }
    } else {
        result = false;
...
/** Starting edit point for billing.
 * @see module:controllers/COBilling~publicStartEdit */
 exports.StartEdit = guard.ensure(['https'], publicStartEdit);
