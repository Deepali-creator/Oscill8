 ...
 */
function start() {
    dw.system.Logger.warn('coshipiing..start...');
    ...
    while (ProductLineItems.hasNext()) {
        ...
        dw.system.Logger.warn('---Product ID---' +pli.product.ID);
       
        
        if(pli.product.ID == 'NT18' && pli.custom.subproSubscriptionSelectedOptionMode == 'onetime' && customer.profile.custom.promocode != null){
            Transaction.wrap(function () {
                customer.profile.custom.promocode = null; 
            });
            break;
        }
    ...
    if (cart.getDefaultShipment().getShippingAddress()) {
        dw.system.Logger.warn("Cart default:"+cart.getDefaultShipment().getShippingAddress().getAddress1());
        dw.system.Logger.warn("cart default shipment");
        //Custom condition added to retain shipping Email in form
        var shippingAddress = cart.getDefaultShipment().getShippingAddress();
        var shippingEmailPresent = true;
        try{
            if(!empty(app.getForm('singleshipping').object.shippingAddress.addressFields.shippingEmail.value))
            {
                var shippingEmail = app.getForm('singleshipping').object.shippingAddress.addressFields.shippingEmail.value;
            }
            else{
                shippingEmailPresent = false;
            }
        }
        catch(e){
            shippingEmailPresent = false;   
        ...
        dw.system.Logger.warn('Here Checking Shipping Email.');
        dw.system.Logger.warn('Value '+shippingEmailPresent);
        dw.system.Logger.warn(shippingAddress.custom.email);
        ...
        session.forms.singleshipping.shippingAddress.addressFields.shippingEmail.value = shippingAddress.custom.email;
        if(shippingEmailPresent)
        {
            dw.system.Logger.warn('Here Shipping Email Present....');
            app.getForm('singleshipping').object.shippingAddress.addressFields.shippingEmail.value = shippingEmail;

        }
    } else {
        if (customer.authenticated && customer.registered && customer.addressBook.preferredAddress) {
            dw.system.Logger.warn('Customer Present....');
            ...
            dw.system.Logger.warn(customer.addressBook.preferredAddress.custom.email);

            app.getForm('singleshipping').object.shippingAddress.addressFields.shippingEmail.value = customer.addressBook.preferredAddress.custom.email;
        }
    }
    // Functionality for changing the shipping method for subscription or non subscription product
    var CheckForSubscriptionProduct = false;
    var plis = cart.getProductLineItems();
    for (var i = 0; i < plis.length; i++) {
        var item = plis[i];
        dw.system.Logger.warn(item.productID);
        if (item.product.custom.subproSubscriptionEnabled) {
            if(item.custom.subproSubscriptionSelectedOptionMode === 'regular') {
                CheckForSubscriptionProduct = true;
                break;
            }
        }
    }
    ...
    var RUPSShippingMethod;
    if(CheckForSubscriptionProduct) {
        session.forms.singleshipping.shippingAddress.shippingMethodID.value = 'UPPP';
        for(var i=0; i < ShippingMethods.length; i++) {
            if(ShippingMethods[i].ID === 'UPPP') {
                Transaction.wrap(function () {
                    cart.getDefaultShipment().setShippingMethod(ShippingMethods[i]);
                });
            }
            else if(ShippingMethods[i].ID === 'RUPS') {
                RUPSShippingMethod = ShippingMethods[i];
            }
        }
    }
    else {
        session.forms.singleshipping.shippingAddress.shippingMethodID.value = 'RUPS';
        for(var i=0; i < ShippingMethods.length; i++) {
            if(ShippingMethods[i].ID === 'RUPS') {
                RUPSShippingMethod = ShippingMethods[i];
                Transaction.wrap(function () {
                    cart.getDefaultShipment().setShippingMethod(ShippingMethods[i]);
                    cart.calculate();
    ...
    var shippingModel = ShippingMgr.getShipmentShippingModel(cart.getShipments()[0]);
    var shippingCost = shippingModel.getShippingCost(RUPSShippingMethod).getAmount();
    // Prepares shipments.
...
function handleShippingSettings(cart) {
    Transaction.wrap(function () {
        ...
        dw.system.Logger.warn("singleshipping:"+ session.forms.singleshipping.shippingAddress.addressFields.shippingEmail.value);
        dw.system.Logger.warn("singleshippingEmail:"+ shippingAddress.custom.email);

        shippingAddress.custom.email = session.forms.singleshipping.shippingAddress.addressFields.shippingEmail.value;
        dw.system.Logger.warn("singleshippingCustomEmail:"+ shippingAddress.custom.email);
...
function updateAddressDetails() {
    dw.system.Logger.warn("Update address details");
    ...
    singleShippingForm.handleAction({
        save: function () {
            ...
            if (customer.authenticated && session.forms.singleshipping.shippingAddress.addToAddressBook.value) {
                dw.system.Logger.warn("Customer authenticated");
