    ...
    formResult = cartForm.handleAction({
        ...
        'addCoupon': function (formgroup) {
            ...
            if (formgroup.couponCode.htmlValue) {
                ...
                var couponCode = formgroup.couponCode.htmlValue;

                dw.system.Logger.warn('---Here Coupon Code--');
                dw.system.Logger.warn(formgroup.couponCode.htmlValue);
                ...
                while (ProductLineItems.hasNext()) {
                    ...
                    if(pli.product.ID == 'NT18' && pli.custom.subproSubscriptionSelectedOptionMode == 'regular') {
                        applyPromo = true;
                        pliItem = pli;
                        break;
                    }
                    else if(pli.product.ID == 'NT18' && pli.custom.subproSubscriptionSelectedOptionMode == 'onetime' && customer.profile.custom.promocode != null){
                ...
                if(customer.authenticated && customer.registered) {
                    ...
                    if(coupon != null) {
                        if(coupon.promotions[0].active) {
                            if(couponCode == 'FBH2021') {
                                if(applyPromo) {
                                    if(!customer.profile.custom.freeProductProvided || customer.profile.custom.freeProductProvided == null) {
                                        Transaction.wrap(function () {
                                            // customer.profile.custom.freeProductProvided = true;
                                            customer.profile.custom.promocode=couponCode;
                                        });
                                        result.CouponError = 'COUPON_APPLIED';
                                    }
                                    else {
                                        result.CouponError = 'COUPON_ALREADY_REDEEM';
                                    }
                                }
                                else {
                                    result.CouponError = 'COUPON_NOT_APPLICABLE';
                                }
                            }
                            else {
                                result.CouponError = 'COUPON_CODE_INVALID';
                            }
                        }
                        else {
                    ...
                    result.CouponError = 'CUSTOMER_NOT_AUTHENTICATED';
                }
                ...
                //     dw.system.Logger.warn('Here I am....');
        ...
        'checkoutCart': function () {
            ...
            dw.system.Logger.warn(JSON.stringify(cart.validateForCheckout()));
            dw.system.Logger.warn(validationResult.EnableCheckout);
        ...
        'deleteCoupon': function (formgroup) {
            ...
            dw.system.Logger.warn('----Here Deleting Coupon----');
            ...
            while (ProductLineItems.hasNext()) {
                var pli = ProductLineItems.next();
                if(pli.product.ID == 'NT18' && pli.custom.subproSubscriptionSelectedOptionMode == 'regular') {
                    dw.system.Logger.warn('---If Before----');
                    dw.system.Logger.warn(customer.profile.custom.promocode);
                    Transaction.wrap(function () {
                        customer.profile.custom.promocode = null;
                        // customer.profile.custom.freeProductProvided = false;
                    });
                    dw.system.Logger.warn('---If After----');
                    dw.system.Logger.warn(customer.profile.custom.promocode);
                }
            }
            return {
            ...
            if (product.isBundle()) {
                ...
                for (j = 0; j < childPids.length; j += 1) {
                    ...
                    if (childProduct) {
                        ...
                        var UpdateProductOptionSelections = require('app_oscill8/cartridge/scripts/cart/UpdateProductOptionSelections');
