...
function getBonusProducts() {
    ...
    var getBonusDiscountLineItemDS = require('app_oscill8/cartridge/scripts/cart/GetBonusDiscountLineItem');
...
function updateSubQyantity() {
    var BasketMgr = require('dw/order/BasketMgr');
    var data = JSON.parse(params.data);
    var basket = BasketMgr.getCurrentOrNewBasket();
    var pli = basket.getAllProductLineItems(data.pliUUID).pop();
    if (!pli) {
        return;
    }
    //var Session = require('dw/system/Session');
    session.custom.enableCheckout = true;
    // var ProductMgr = require('dw/catalog/ProductMgr');
    // var product = ProductMgr.getProduct('NT18RP');
    // if (product === null || product.online) {
    //     session.custom.enableCheckout = false;
    // }
    // var availabilityLevels = product.getAvailabilityModel().getAvailabilityLevels(parseInt(data.quantity));
    // session.custom.enableCheckout = session.custom.enableCheckout && (availabilityLevels.getNotAvailable().value === 0);
    require('dw/system/Transaction').wrap(function () {
        dw.system.Logger.warn('---Updating Quantity----');
        dw.system.Logger.warn(data.quantity);
        dw.system.Logger.warn(parseInt(data.quantity));
        pli.custom.subscriptionQuantity = parseInt(data.quantity);
    });
    dw.system.Logger.warn('---After Updating-----');
    dw.system.Logger.warn(pli.custom.subscriptionQuantity);
}
...
/**
 * Upadte a custom subscription quantity field on productLineItem
 * @see module:controllers/Product~updateSubQyantity
 */
exports.UpdateSubQyantity = guard.ensure(['post'], updateSubQyantity);
