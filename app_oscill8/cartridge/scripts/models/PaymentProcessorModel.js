...
PaymentProcessorModel.handle = function (cart, paymentMethodID) {
    ...
    dw.system.Logger.warn(processor.ID);
    dw.system.Logger.warn('&&&&&&&&'+dw.system.HookMgr.hasHook('app.payment.processor.' + processor.ID));
    if (dw.system.HookMgr.hasHook('app.payment.processor.' + processor.ID)) {
        Logger.warn('---------Processor ID----' +processor.ID);
        // return "stringg if";
...
PaymentProcessorModel.authorize = function (order, paymentInstrument) {
    ...
    dw.system.Logger.warn("++++++++Processor++++++++++++="+processor);
    if (HookMgr.hasHook('app.payment.processor.' + processor.ID)) {
        dw.system.Logger.warn("++++++++In hook++++++++++++");
        var result =  HookMgr.callHook('app.payment.processor.' + processor.ID, 'Authorize', {
            Order: order,
            OrderNo: order.getOrderNo(),
            PaymentInstrument: paymentInstrument
        });

        dw.system.Logger.warn("-----------Hook result-----------------"+JSON.stringify(result));
        return  result;
    } else {
