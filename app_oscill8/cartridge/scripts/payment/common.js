...
function validatePaymentInstruments(paymentContainer, countryCode, amount) {
    ...
    dw.system.Logger.warn("@@@@@@@@@@@@ paymentContainer @@@@@@@@@@"+JSON.stringify(paymentContainer));

    var paymentInstruments = paymentContainer.getPaymentInstruments();
    dw.system.Logger.warn("@@@@@@@@@@@@  paymentInstruments @@@@@@@@@@"+paymentInstruments);
    // Gets applicable payment methods.
    var methods = PaymentMgr.getApplicablePaymentMethods(customer, countryCode, amount);

    dw.system.Logger.warn("@@@@@@@@@@@@  methodsArray @@@@@@@@@@"+methods);
    ...
    for (var i = 0; i < paymentInstruments.length; i++) {
        ...
        dw.system.Logger.warn('--------Payment Instrument in Common.js--------' +paymentInstruments[i].paymentMethod);
    ...
    if (invalidPaymentInstruments.size()) {
        var i = 0;
        dw.system.Logger.warn('%%%%%%%%%%%%'+invalidPaymentInstruments[i]);
        i++;
        return {
